...
object GamepadHelper {
    ...
    fun getAxisName(axis: Int, direction: Int?): String =
        "Axis " + axis + direction?.let { if (it > 0) "+" else "-" }

    private fun toTitleCase(raw: String): String = raw.replace("_", " ").lowercase()
        .split(" ").joinToString(" ") { it.replaceFirstChar { c -> c.uppercase() } }

    private data class DefaultButtonMapping(val setting: InputMappingSetting, val hostKeyCode: Int)
    ...
    fun applyAutoMapBindings(settings: Settings, isNintendoLayout: Boolean, useAxisDpad: Boolean) {
        val faceButtons =
            if (isNintendoLayout) nintendoFaceButtonMappings else xboxFaceButtonMappings
    ...
    fun getJoystickComponent(buttonType: Int): JoystickComponent? = when (buttonType) {
        ...
        NativeLibrary.ButtonType.STICK_LEFT_DOWN -> JoystickComponent(
            NativeLibrary.ButtonType.STICK_LEFT,
            true
        )

        NativeLibrary.ButtonType.STICK_LEFT_LEFT,
        NativeLibrary.ButtonType.STICK_LEFT_RIGHT -> JoystickComponent(
            NativeLibrary.ButtonType.STICK_LEFT,
            false
        )

        NativeLibrary.ButtonType.STICK_C_UP,
        NativeLibrary.ButtonType.STICK_C_DOWN -> JoystickComponent(
            NativeLibrary.ButtonType.STICK_C,
            true
        )

        NativeLibrary.ButtonType.STICK_C_LEFT,
        NativeLibrary.ButtonType.STICK_C_RIGHT -> JoystickComponent(
            NativeLibrary.ButtonType.STICK_C,
            false
        )
    ...
    val hotKeys = listOf(
        ...
        InputMappingSetting.HOTKEY_COMBO_BUTTON
    )
